...
public class ShellCoreService : IShellCoreService
{
    ...
    public async Task StartAsync()
    {
        ...
        try
        {
            ShellLogger.Info(nameof(ShellCoreService), "Starting ShellCoreService.");
            ...
            // Start UI Host (skip in test mode to avoid full-screen UI during automated runs)
            if (!ShellConfiguration.IsTestMode)
            {
                await StartUiHostAsync();
            }

            State = ServiceState.Running;
            ShellLogger.Info(nameof(ShellCoreService), "ShellCoreService started successfully.");
        }
        catch (Exception ex)
        {
            State = ServiceState.Failed;
            ShellLogger.Error(nameof(ShellCoreService), "Failed to start ShellCoreService.", ex);
            await CleanupAsync();
    ...
    public async Task StopAsync()
    {
        ...
        try
        {
            ...
            ShellLogger.Info(nameof(ShellCoreService), "ShellCoreService stopped successfully.");
        }
        catch (Exception ex)
        {
            State = ServiceState.Failed;
            ShellLogger.Error(nameof(ShellCoreService), "Failed to stop ShellCoreService.", ex);
    ...
    public void Dispose()
    {
        ...
        _disposed = true;
        ShellLogger.Info(nameof(ShellCoreService), "ShellCoreService disposed.");
    }
}
