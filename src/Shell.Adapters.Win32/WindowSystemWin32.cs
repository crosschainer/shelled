...
public class WindowSystemWin32 : IWindowSystem, IDisposable
{
    ...
    public ShellWindow? GetWindowInfo(IntPtr hwnd)
    {
        ...
        try
        {
            ...
            // Derive a more stable app identifier. We prefer the
            // process executable name when available so Chromium/
            // Electron-based apps (which often share a class name)
            // are grouped independently on the taskbar.
            var appId = className;
            try
            {
                if (processId != 0)
            ...
            {
                // Best-effort only; fall back to class name if we
                // can't resolve the process or module path.
            }
