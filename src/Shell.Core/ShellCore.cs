...
public class ShellCore : IDisposable
{
    ...
    public ShellState GetState()
    {
        ...
        return new ShellState
        {
            ...
            LauncherApps = _state.LauncherApps.ToDictionary(
                kvp => kvp.Key,
                kvp => new LauncherApp
                {
                    Id = kvp.Value.Id,
                    ...
                    ExecutablePath = kvp.Value.ExecutablePath,
            ...
            DesktopItems = _state.DesktopItems
                .Select(item => new DesktopItem
                {
                    Name = item.Name,
                    Path = item.Path,
    ...
    /// Minimize a window
    /// </summary>
    public bool MinimizeWindow(IntPtr windowHandle)
    {
        ...
        _windowSystem.ShowWindow(windowHandle, WindowState.Minimized);

        _eventPublisher.Publish(new WindowStateChangedEvent(windowHandle, previousState, WindowState.Minimized));
        _eventPublisher.Publish(new WindowUpdatedEvent(window));
    ...
    public bool RestoreWindow(IntPtr windowHandle)
    {
        ...
        _windowSystem.ShowWindow(windowHandle, WindowState.Normal);
        _windowSystem.SetForegroundWindow(windowHandle);

        _eventPublisher.Publish(new WindowStateChangedEvent(windowHandle, previousState, WindowState.Normal));
        _eventPublisher.Publish(new WindowUpdatedEvent(window));
    ...
    private void InitializeState()
    {
        ...
        InitializeLauncherAppsFromStartMenu();
    }

    /// <summary>
    /// Populate launcher apps from the Windows Start Menu in addition to the
    /// built-in defaults configured on ShellState.
    /// </summary>
    private void InitializeLauncherAppsFromStartMenu()
    {
        try
        {
            ...
            void AddShortcutsFrom(string? root)
            {
                ...
                foreach (var shortcutPath in Directory.EnumerateFiles(root, "*.lnk", SearchOption.AllDirectories))
                {
                    ...
                    // Avoid obvious maintenance/uninstall entries which are not
                    // typically surfaced prominently in the Start menu.
                    if (name.Contains("uninstall", StringComparison.OrdinalIgnoreCase))
                    {
                        continue;
                    }

                    // Use the full shortcut path as a stable, unique ID.
                    var id = shortcutPath;

                    if (_state.LauncherApps.ContainsKey(id))
                    {
                        continue;
                    }

                    _state.LauncherApps[id] = new LauncherApp
                    {
                        Id = id,
                        Name = name,
            ...
            AddShortcutsFrom(userPrograms);
            AddShortcutsFrom(commonPrograms);
        ...
        catch (Exception ex)
        {
            ...
            {
                Console.WriteLine($"Error initializing launcher apps from Start menu: {ex.Message}");
            }
        }
    }
