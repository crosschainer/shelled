...
- Depends only on simple interfaces such as `IWindowSystem`,
  `IProcessLauncher`, `ITrayHost`, and `IHotkeyService`.
...
- `IHotkeyService` registers and unregisters hotkeys, routing them to Core
  commands.
...
shell-registration side effects during testing.
...
- Loads the bundled Web UI from `Shell.UI.Web` and exposes a `window.shell`
  bridge object in JS.
- Serializes Core events into JSON and delivers them via `PostWebMessage`.
- Deserializes JS commands (`focusWindow`, `launchApp`, `switchWorkspace`,
  `trayIconInvoke`, etc.) and forwards them to Core command handlers.
...
- Native components (`Shell.Core`, adapters, WebView host) are built as a single
  solution (likely .NET + C++/Win32 as we flesh out the details).
